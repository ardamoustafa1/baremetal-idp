...
| `clusterissuers.yaml.tpl` (resources/ DIŞINDA — ArgoCD'nin directory sync'i `.tpl` dosyalarını almaz, bkz. test-certificate.yaml.tpl ile aynı desen) (envsubst render edilir → `rendered/clusterissuers.yaml`) | 3 ClusterIssuer: `vault-issuer-{dev,staging,prod}` |
